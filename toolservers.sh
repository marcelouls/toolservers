...
while [ $OPTION -ne 9 ]; do
	...
	read -p $'\033[32;1mDigite opção: \033[m' CLIENTE
	...
	then	 
		LINK_LIS="bd_links.txt"
		# echo  "$MENU"
		#echo -e "                    \e[1;32m $SUBTITTLE \e[0m";
		read -p $'\033[32;1mCód. do cliente: \033[m' CLIENTE

		echo "CLIENTE  AMBIENTE  LINK                                         USUARIO       SENHA"
		grep $CLIENTE $LINK_LIS | while read -r line ; do
		echo "$line"
		# your code goes here
	done
		...
		if [[ ! -z $SERVER ]]; then
			...
			then
				...
				then
					...
					$PRINTPASS
					$CONECTA 'export AMBIENTE='$AMB '; tomcatctl contexto' 2>&1;
					echo -e "\n";
			...
			then
				clear
				menu-tomcat
				read -p $'\033[32;1mDigite Opção desejada : \033[m' REINICIA ;
				if [ $REINICIA == "1" ]
				then 
					$PRINTPASS
				...
				then
					$PRINTPASS
					$CONECTA 'export AMBIENTE='$AMB '; tomcatctl contexto';
					read -p $'\033[32;1mInforme numero de tomcat : \033[m' NUMTOM 
			...
			then
				...
				###################### pass pacote ############
			then
				echo " Uso exclusivo de Infra " ;
				read -p $'\033[32;1mDigite a senha : \033[m' PASSINF

				while [ ! $PASSINF == "07052018" ]; do
					echo "senha errada"; exit 1;
				done
				...
				echo "     ------------------------------------------";
				read -p $'\033[32;1mDigite a opção desejada : \033[m' PACOTE ;
				if [ $PACOTE == "1" ]
				then
					$PRINTPASS
					$CONECTA "ls -t /MV/apps/soulmv_'$AMB'/products/soul-product-forms/" ;
					read -p $'\033[32;1mEscolha a versão : \033[m'  VERSAO;
					read -p $'\033[32;1mInforme o produto (ex. ffcv) : \033[m' PRDT;
				...
				then
					...
					read -p $'\033[32;1mInforme a versão : \033[m'  VERSAO;
					read -p $'\033[32;1mInforme o produto (ex. ffcv) : \033[m' PRDT;
					read -p $'\033[32;1mInforme o nome da tela (Camel case) : \033[m' TELA
				...
				then
					ls -l soul-*;
					read -p $'\033[32;1mCopie e cole o nome do arquivo : \033[m' ARQUIVO
					$PRINTPASS
					$CONECTA "ls -t /MV/apps/soulmv_'$AMB'/products/soul-product-forms/" ;
					read -p $'\033[32;1mInforme a versão : \033[m'  VERSAO;
